...
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-white uppercase tracking-tight">Novo Nível de Acesso</h1>
        <p class="text-slate-400 mt-1">Defina o nome, slug e as permissões habilitadas.</p>
    </div>

    <form action="/roles/store" method="POST" class="space-y-8">
        <div class="bg-slate-800 rounded-3xl border border-slate-700 shadow-2xl p-8 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nome" placeholder="Ex: Supervisor" required class="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase mb-2">SLUG (Identificador único)</label>
                    <input type="text" name="slug" placeholder="ex: supervisor" required class="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Descrição</label>
                    <textarea name="descricao" rows="2" class="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white outline-none focus:ring-2 focus:ring-blue-500 transition-all"></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-800 rounded-3xl border border-slate-700 shadow-2xl p-8">
            <h3 class="text-lg font-bold text-white flex items-center mb-6">
                <i class="fa-solid fa-lock mr-3 text-blue-500"></i> Permissões Disponíveis
            </h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <label class="flex items-center p-4 bg-slate-900 rounded-2xl border border-slate-700 hover:border-blue-500/50 cursor-pointer transition-all group">
                    <div class="relative flex items-center">
                        <input type="checkbox" name="permissions[]" value="<?= $perm['id'] ?>" class="w-5 h-5 rounded border-slate-700 bg-slate-800 text-blue-600 focus:ring-blue-500">
                    </div>
                    <div class="ml-4">
